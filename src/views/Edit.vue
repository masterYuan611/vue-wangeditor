  ...
  <div class="create-idea">
    <div class="content">
      ...
      <div class="function">
        ...
        <Button @click="save" :disabled="buttonDisabled" v-if="['0', '3'].includes(status)">保存草稿</Button>
...
import { allTemplate, viewSave, viewSubmitAudit, viewDetail } from '../api';
...
export default {
  ...
  data() {
    return {
      viewId: this.$route.query.viewid, // 观点id
      viewTitle: '', // 标题
      editor: null, // 富文本编辑器
  ...
  created() {
    this.getAlltemplate(); // 获取所有观点模板
    this.viewDetail();
    this.$EventBus.$on('editSubmit', () => {
      this.submit();
    });
    this.$EventBus.$on('editSave', () => {
      this.save();
    });
    this.$EventBus.$on('editReset', () => {
      this.reset();
    });
  },
  mounted() {
  ...
  methods: {
    ...
    viewDetail() {
      viewDetail({ viewId: this.viewId }).then(({ data }) => {
        this.status = data.status; // 观点状态
        if (data.status === '4') {
          // 已上架更新已驳回的观点，点击编辑需要展示更新后的内容
          this.viewTitle = data.updateViewTitle;
          this.editor.txt.html(data.updateViewContent);
        } else {
          this.viewTitle = data.viewTitle; // 观点标题
          this.editor.txt.html(data.viewContent);
        }
      });
    },
    // 选择模板
